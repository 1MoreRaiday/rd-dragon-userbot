...
async def help_cmd(_, message: Message):
    if len(message.command) == 1:
        ...
        text = (
            "<b>Help for <a"
            " href=https://t.me/Dragon_Userbot_chat>Dragon-Userbot</a>\nFor"
            f" more help on how to use a command, type <code>{prefix}help"
            " [module]</code>\n\nAvailable Modules:\n"
        )
            ...
            for command in commands.keys():
                if command.split()[0] == command_name:
                    ...
                    return await message.edit(
                        "<b>Help for command"
                        f" <code>{prefix}{command_name}</code>\nModule:"
                        f" {name} (<code>{prefix}help"
                        f" {name}</code>)</b>\n\n<code>{prefix}{cmd[0]}</code>{' <code>' + cmd[1] + '</code>' if len(cmd) > 1 else ''} —"
                        f" <i>{cmd_desc}</i>"
                    )
